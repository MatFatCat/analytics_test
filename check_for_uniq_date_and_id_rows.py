...
milk_data = pd.read_csv("/Users/nikolamuravev/Desktop/milk_data(labels 50-100M).csv", encoding="utf-8",
                        on_bad_lines="skip")
milk_data.reset_index(drop=True)

labels = pd.read_csv("/Users/nikolamuravev/Desktop/labels_50M_to_100M.csv", encoding="cp1252",
                        on_bad_lines="skip")
...
def row_handler(row):
    try:
        ...
        print("Something wrong with value")
        print(labels_row_check["item_id"])


milk_data.apply(lambda row: row_handler(row), axis=1)
new_df.to_csv("/Users/nikolamuravev/Desktop/final_dataset(50-100).csv")
